...
# --- Pass build variables through to the chroot hooks -----------------------
log "Writing /etc/aptura/build.env into the image"
ENV_DST="${WORK}/config/includes.chroot/etc/aptura"
mkdir -p "${ENV_DST}"
cat > "${ENV_DST}/build.env" <<EOF
# Generated by build-iso.sh from config/distro.env. Read by chroot hooks.
DEFAULT_LOCALE="${DEFAULT_LOCALE}"
SECONDARY_LOCALE="${SECONDARY_LOCALE}"
DEFAULT_LANGUAGE="${DEFAULT_LANGUAGE}"
DEFAULT_KEYBOARD_LAYOUT="${DEFAULT_KEYBOARD_LAYOUT}"
DEFAULT_KEYBOARD_VARIANT="${DEFAULT_KEYBOARD_VARIANT}"
DEFAULT_KEYBOARD_MODEL="${DEFAULT_KEYBOARD_MODEL}"
DEFAULT_TIMEZONE="${DEFAULT_TIMEZONE}"
PREFER_FLATPAK_OVER_SNAP="${PREFER_FLATPAK_OVER_SNAP}"
ENABLE_FLATPAK="${ENABLE_FLATPAK}"
ENABLE_UNATTENDED_UPGRADES="${ENABLE_UNATTENDED_UPGRADES}"
SIGN_KERNEL="${SIGN_KERNEL:-false}"
EOF
